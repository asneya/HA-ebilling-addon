...
async def export_config():
    """Descarga los ajustes y las tarifas para llevarlos a Vatia.

    Este add-on ha pasado a llamarse **Vatia** y, al cambiar el slug, Home
    Assistant lo trata como uno nuevo con su propio ``/data``: la configuración
    no viaja sola. Esta versión existe solo para poder sacarla de aquí en un
    clic, sin entrar por SSH al sistema de ficheros. Incluye los secretos porque
    si no, no restaura del todo.
    """
    config = storage.load()
    payload = {
        "app": "vatia",
