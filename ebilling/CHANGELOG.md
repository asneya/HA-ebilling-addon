...
## 0.20.1 — versión puente

Este add-on ha pasado a llamarse **Vatia** y sigue su desarrollo allí. Esta
versión **no trae funciones nuevas**: existe solo para que puedas llevarte la
configuración sin entrar por SSH al sistema de ficheros.

Al cambiar el `slug`, Home Assistant trata Vatia como un add-on distinto y le da
su propio `/data`, así que los ajustes y las tarifas no viajan solos.

1. Abre este add-on (**Migrar a Vatia** en la barra lateral) → **Ajustes**.
2. Pulsa **Descargar la configuración**.
3. Instala **Vatia** e impórtalo en **Ajustes → Copia de seguridad**.
4. Vuelve a poner el token de Home Assistant en *Fuente de datos* si no viene.
5. Comprueba que todo está en su sitio y **desinstala este add-on**.

El fichero descargado incluye el token de Home Assistant: trátalo como una
contraseña y bórralo cuando acabes.
